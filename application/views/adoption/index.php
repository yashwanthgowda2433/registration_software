...
<div class="container">
	<div class="row heading-row">
		<div class="col-sm-10 text-center mx-auto mt-5">
            ...
            <h2 style='font-family: "Tw Cen MT";color:#000;'>Adoption Lists</h2>
        </div>
		<div class="col-sm-2 mx-auto mt-5">
		<?php if($user->role == "Admin" || $user->role == "SuperAdmin"){ ?>
		    <a class="btn btn-dark" style="width:150px;height: max-content;margin-top: auto;margin-bottom: auto;" href="<?php echo base_url('adoption/add');?>">Add</a>	
		<?php } ?>
				
	    </div>
	</div>
    <form method="get" action="<?php echo base_url('adoption/index');?>">
    ...
    <div class="row">
        <div class="col-sm-10">
		    <div class="row heading-row">
				...
				<div class="col-sm-3 mt-1">
			        <div class="form-control">
					    <input type="text" class="form-input" placeholder="Child Name" value="<?= $this->input->get('col13')?$this->input->get('col13'):''?>" name="col13" style="width:100%;"/>
			        </div>
		        </div>
				<div class="col-sm-3 mt-1">
			        <div class="form-control">
				...
				<div class="col-sm-3 mt-1">
			        <div class="form-control">
					    <input type="text" class="form-input" placeholder="Party" value="<?= $this->input->get('col5')?$this->input->get('col5'):''?>" name="col5" style="width:100%;"/>
			        </div>
		        </div>
				<div class="col-sm-3 mt-1">
			        <div class="form-control">
		...
		<div class="col-sm-1">
            <div class="row heading-row" style="min-height: 100%;">
				  ...
				  <button class="btn btn-danger" onclick="window.location.href='<?php echo base_url('/adoption');?>'" style="width:150px;height: max-content;margin-top: auto;margin-bottom: auto;" type="button">Reset</button>
            </div>
        </div>
    </div>
    </form>
        ...
        <div class="module-content-section">
	        <div class="table-header-data">
	            ...
	            <div class="left">Showing <?php echo $page_size>sizeof($cadet)?sizeof($cadet):$page_size; ?> of <?php echo $total; ?> | Page <?php echo $current_page; ?> of <?php echo $total_pages; ?></div>
	                <div class="right">
					    Records Per Page:  &nbsp; <select name="page_size" class="filter-form-select" style="width:60px;" onchange="window.location.href='<?php echo site_url("adoption/index?".$query_string_page_size); ?>&page_size='+$(this).val()">
	                ...
	                <div class="table-container">
						<table>
						  ...
						  <?php if($user->role == "Admin" || $user->role == "SuperAdmin"){ ?>
								...
								<td><a href="#" onclick="searchFile('adoption','<?php echo $l->pdf_link; ?>')"><?php echo $l->pdf_link; ?></a></td>
								
								<td>
								    <a href="<?php echo base_url('adoption/edit/'.$l->id); ?>" class="" title="Edit Listing"><i class="fas fa-pencil-alt" style=" font-size:1.3em; position:relative; top:2px; color:#ffb800;"></i> Edit</a>
									<?php if($user->role == "Admin" || $user->role == "SuperAdmin"){ ?>
						...
						<div class="data-pagination">
							<?php if($prev_page): ?>
							<a href="<?php echo site_url("adoption/index?page=".$prev_page_num."&".$query_string_pagination); ?>" class="form-button small-button pagination-buttons">Previous</a>
							<?php endif; ?>
							...
							<?php
								...
								if($total_pages>14 and $current_page==$total_pages){ $start=$current_page-10;  $total=($total+$start)>$total_pages?$total_pages:($total+$start); }
									...
									for($i=$start; $i<=$total; $i++){
										...
										if(($i)!=$current_page){
									?>
										<a href="<?php echo site_url("adoption/index?page=".($i-1)."&".$query_string_pagination); ?>" class="form-button small-button pagination-buttons"><?php echo $i; ?></a>
									<?php
								 } else {
									 echo "&nbsp;".$i."&nbsp;";
									 }
							...
							<?php if($next_page): ?>
							<a href="<?php echo site_url("adoption/index?page=".$next_page_num."&".$query_string_pagination); ?>" class="form-button small-button pagination-buttons">Next</a>
							<?php endif; ?>
						</div>
                    </div>
		        </div>
...
<script>
	function deleteAll(){
		...
		window.location.href = "<?php echo base_url('adoption/delete/');?>"+delete_ids;
	}
