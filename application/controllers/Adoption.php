...
class Adoption extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Adoption_model','adoption');
	...
	public function index()
	{
	  ...
	  if($cadet){
		...
		$view_params['col13'] = $this->input->get('col13');
		...
		// if(!empty($view_params['year_of_admn']) || !empty($view_params['year_of_discharged'] || !empty($view_params['cadet_no']))){
		    $cadet = $this->adoption->view($view_params);
			// print_r($cadet);die;

		    $count = $this->adoption->count($view_params);
		// }
		...
		$data['total']=$count;
			...
			$data['district'] = $this->adoption->get_district();
			$data['taluk'] = $this->adoption->get_taluk();


		$this->load->view('includes/header');
		$this->load->view('adoption/index',$data);
	...
	public function add()
	{
	  ...
	  if($cadet){
		...
		if(!empty($this->input->post('col1')))
		{
			...
			$data['taluk'] = $this->adoption->get_taluk();
			$this->load->view('includes/header');
		    $this->load->view('adoption/add',$data);
		    $this->load->view('includes/footer');
		}
	  }else{
		redirect(base_url('login/'));
	...
	public function edit($id)
	{
		...
		if(!empty($this->input->post('col1')))
		{
		    ...
		    $data = $this->adoption->edit($addparams);
			if($data)
		    {
			    $this->session->set_flashdata('success','Successfully submitted');
			    redirect(base_url('adoption/index'));
		...
		//    print_r($cadet);die;
		   ...
		   if(!empty($cadet->col2))
		   {
			    ...
			    $data['taluk'] = $this->adoption->get_taluk();
				$this->load->view('includes/header');
		        $this->load->view('adoption/edit',$data);
		        $this->load->view('includes/footer');
	...
	public function delete($id)
	{
		if($id){
			...
			foreach($id_arr as $data){
			    $addparams['id'] = $data;
		        $data = $this->adoption->delete($addparams);
				if($data)
				{
					$success=$success+1;
				}else{
