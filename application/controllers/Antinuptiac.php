...
class Antinuptiac extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Antinuptiac_model','antinuptiac');
	...
	public function index()
	{
	  ...
	  if($cadet){
		...
		// if(!empty($view_params['year_of_admn']) || !empty($view_params['year_of_discharged'] || !empty($view_params['cadet_no']))){
		    $cadet = $this->antinuptiac->view($view_params);
			// print_r($cadet);die;

		    $count = $this->antinuptiac->count($view_params);
		// }
		...
		$data['total']=$count;
			...
			$data['district'] = $this->antinuptiac->get_district();
			$data['taluk'] = $this->antinuptiac->get_taluk();
		$this->load->view('includes/header');
		$this->load->view('antinuptiac/index',$data);
	...
	public function add()
	{
	  ...
	  if($cadet){
		...
		if(!empty($this->input->post('col1')))
		{
		    ...
		    $data = $this->antinuptiac->add($addparams);
			if($data)
		    {
			    $this->session->set_flashdata('success','Successfully submitted');
			    redirect(base_url('antinuptiac/index'));
		    }else{
			...
			$data['taluk'] = $this->antinuptiac->get_taluk();
			$this->load->view('includes/header');
		    $this->load->view('antinuptiac/add',$data);
		    $this->load->view('includes/footer');
		}
	  }else{
		redirect(base_url('login/'));
	...
	public function edit($id)
	{
		...
		if(!empty($this->input->post('col1')))
		{
		    ...
		    $data = $this->antinuptiac->edit($addparams);
			if($data)
		    {
			    $this->session->set_flashdata('success','Successfully submitted');
			    redirect(base_url('antinuptiac/index'));
		...
		//    print_r($cadet);die;
		   ...
		   if(!empty($cadet->col2))
		   {
				...
				$this->load->view('includes/header');
		        $this->load->view('antinuptiac/edit',$data);
		        $this->load->view('includes/footer');
	...
	public function delete($id)
	{
		if($id){
			...
			foreach($id_arr as $data){
			    $addparams['id'] = $data;
		        $data = $this->antinuptiac->delete($addparams);
				if($data)
				{
					$success=$success+1;
				}else{
