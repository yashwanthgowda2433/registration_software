...
class Deeds extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Deeds_model','deeds');
	...
	public function index()
	{
	  ...
	  if($cadet){
		...
		// if(!empty($view_params['year_of_admn']) || !empty($view_params['year_of_discharged'] || !empty($view_params['cadet_no']))){
		    $cadet = $this->deeds->view($view_params);
			// print_r($cadet);die;

		    $count = $this->deeds->count($view_params);
		// }
		...
		$this->load->view('deeds/index',$data);
	...
	public function edit($id)
	{
		...
		if(!empty($this->input->post('col1')))
		{
		    ...
		    $data = $this->deeds->edit($addparams);
			if($data)
		    {
			    $this->session->set_flashdata('success','Successfully submitted');
			    redirect(base_url('deeds/index'));
		...
		//    print_r($cadet);die;
		   ...
		   if(!empty($cadet->col2))
		   {
				...
				$this->load->view('includes/header');
		        $this->load->view('deeds/edit',$data);
		        $this->load->view('includes/footer');
	...
	public function delete($id)
	{
		if($id){
			...
			foreach($id_arr as $data){
			    $addparams['id'] = $data;
		        $data = $this->deeds->delete($addparams);
				if($data)
				{
					$success=$success+1;
				}else{
