...
class Wills extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('Wills_model','wills');
	...
	public function index()
	{
	  ...
	  if($cadet){
		...
		// if(!empty($view_params['year_of_admn']) || !empty($view_params['year_of_discharged'] || !empty($view_params['cadet_no']))){
		    $cadet = $this->wills->view($view_params);
			// print_r($cadet);die;

		    $count = $this->wills->count($view_params);
		// }
		...
		$data['total']=$count;
			...
			$data['district'] = $this->wills->get_district();
			$data['taluk'] = $this->wills->get_taluk();
		$this->load->view('includes/header');
		$this->load->view('wills/index',$data);
	...
	public function add()
	{
	  ...
	  if($cadet){
		...
		if(!empty($this->input->post('col1')))
		{
		    ...
		    $data = $this->wills->add($addparams);
			if($data)
		    {
			    $this->session->set_flashdata('success','Successfully submitted');
			    redirect(base_url('wills/index'));
		    }else{
	...
	public function edit($id)
	{
		...
		if(!empty($this->input->post('col1')))
		{
		    ...
		    $data = $this->wills->edit($addparams);
			if($data)
		    {
			    $this->session->set_flashdata('success','Successfully submitted');
			    redirect(base_url('wills/index'));
		...
		//    print_r($cadet);die;
		   ...
		   if(!empty($cadet->col2))
		   {
				...
				$this->load->view('includes/header');
		        $this->load->view('wills/edit',$data);
		        $this->load->view('includes/footer');
	...
	public function delete($id)
	{
		if($id){
			...
			foreach($id_arr as $data){
			    $addparams['id'] = $data;
		        $data = $this->wills->delete($addparams);
				if($data)
				{
					$success=$success+1;
				}else{
